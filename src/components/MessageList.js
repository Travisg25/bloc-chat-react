...
class MessageList extends Component {
  ...
  createMessage(e) {
    e.preventDefault();
    let participant = this.props.firebase
      .database()
      .ref("rooms/" + this.props.activeRoom + "/participants");
    let messagesRef = this.props.firebase
      .database()
  ...
  editMessage(message) {
    let editMessage = (
      <form onSubmit={this.updateMessage}>
        ...
        <button type="button" onClick={() => this.setState({ toEdit: "" })}>
          Cancel
        </button>
      </form>
    );
  ...
  render() {
    ...
    let currentMessages = this.state.messages.map(message => {
          ...
          <li key={message.key}>
            ...
            this.props.user === message.username ? (
              ...
              <div>
                ...
                <button onClick={() => this.setState({ toEdit: message.key })}>
                  Edit
                </button>
              </div>
            )}
